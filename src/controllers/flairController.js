...
module.exports = {
  ...
  new(req, res, next){
     res.render("flairs/new", {topicpostId: req.params.topicId});
   },
   ...
   create(req, res, next){
     ...
     flairQueries.addFlair(newTopic, (err, flair) => {
       if(err){
   ...
   update(req, res, next){
     flairQueries.updateFlair(req.params.id, req.body, (err, flair) => {
       if(err || flair == null){
         res.redirect(404, `/posts/${req.params.topicId}/flairs/${req.params.id}/edit`);
       } else {
         res.redirect(`/posts/${req.params.topicId}/flairs/${req.params.id}`);
       }
     });
   }
